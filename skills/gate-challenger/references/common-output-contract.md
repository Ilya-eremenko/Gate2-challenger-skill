...
- `stage_checklist`: selected-stage mandatory checklist rendered as green/yellow/red traffic-light items
...
- `evidence`: compact source proof when green, the partial/insufficient proof when yellow, or the missing item when red
...
- Use `yellow` when the item is partially supported, only planned, only named as a heading, or not substantiated by enough evidence.
- Use `red` only when the item is absent completely.
...
For 2+ Stream Review, map these fields explicitly in the final synthesis:

- `approval_scope` is the stream continuation, backlog change, resource ask, financial monitoring, or roadmap step that current plan / fact evidence makes safe to approve now
- `not_approved_scope` is the broader scale, PMF, Gate 4, baseline transfer, resource load, or financial uplift not yet proven by current-period plan / fact evidence
- `next_gate_conditions` may be rendered as `next_sr_conditions`

For Progress Review, map these fields explicitly in the final synthesis:

- `approval_scope` is the stream continuation, scope change, resource ask, financial monitoring, or roadmap step that current plan / fact evidence makes safe to approve now
- `not_approved_scope` is the broader scale, resource load, financial uplift, or continuation not yet proven by current-period plan / fact evidence and explicit stop criteria
- `next_gate_conditions` may be rendered as `next_review_conditions`
